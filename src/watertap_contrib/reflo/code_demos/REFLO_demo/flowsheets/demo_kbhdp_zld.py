...
def add_zld_mec(m, permian_cryst_config):
    ...
    treat.ro_to_product_NaCl_mixer = Arc(
        source=treat.NaCl_cryst_product_translator.outlet,
...
def add_zld_treatment_costing(m, heat_price, electricity_price):
    ...
    constraint_scaling_transform(
        m.fs.treatment.EC.ec.costing.capital_cost_electrodes_constraint, 1e-3
    )
    ...
    constraint_scaling_transform(
        m.fs.treatment.mec.unit.costing.total_capital_cost_effect_1_constraint, 1e-7
    )
    constraint_scaling_transform(
        m.fs.treatment.mec.unit.costing.total_capital_cost_effect_2_constraint, 1e-7
    )
    constraint_scaling_transform(
        m.fs.treatment.mec.unit.costing.total_capital_cost_effect_3_constraint, 1e-7
    )
    constraint_scaling_transform(
        m.fs.treatment.mec.unit.costing.total_capital_cost_effect_4_constraint, 1e-7
    )
...
def add_zld_heat_energy_costing(
    m,
...
def add_zld_electricity_energy_costing(
    m, cost_per_watt_installed, heat_price, electricity_price
):
    energy = m.fs.energy
...
def zld_main(
    ...
    if treatment_only == True:
        ...
        add_zld_treatment_costing(
            m, heat_price, electricity_price, nacl_recovery_price=nacl_recovery_price
        )
        ...
        print(
            "MEC heat:",
            pyunits.convert(
                m.fs.treatment.mec.unit.effects[1].effect.work_mechanical[0],
                to_units=pyunits.kW,
            )(),
        )
...
def recovery_check(m):
    ...
    print(
        "MD SW to NaCl:",
        m.fs.treatment.sw_to_nacl_disposal.outlet.flow_mass_phase_comp[
            0, "Liq", "H2O"
        ](),
    )
    ...
    print(f"MEC Recovery: {100 * (value(mec_product) / value(mec_feed)):<5.2f}%")

    # System recovery
    feed_density = 1000 * pyunits.kg / pyunits.m**3
    system_feed = pyunits.convert(
        m.fs.treatment.feed.properties[0].flow_mass_phase_comp["Liq", "H2O"]
        / feed_density,
        to_units=pyunits.m**3 / pyunits.s,
    )
...
if __name__ == "__main__":
    ...
    for effect_number, eff in m.fs.treatment.mec.unit.effects.items():
        print(
            effect_number,
            value(eff.effect.properties_solids[0].flow_mass_phase_comp["Sol", "NaCl"]),
            pyunits.get_units(
                eff.effect.properties_solids[0].flow_mass_phase_comp["Sol", "NaCl"]
            ),
        )
    ...
    print(
        "\nFeed Basis Treatment SEC (heat) in kWh/m3:",
        m.fs.treatment.costing.aggregate_flow_heat() / feed_m3h(),
    )
    print(
        "Feed Basis Treatment SEC (electricity) in kWh/m3:",
        m.fs.treatment.costing.aggregate_flow_electricity() / feed_m3h(),
    )

    print("\nThermal SEC Breakdown")
    print(
        "VAGMD SEC (heat):",
        m.fs.treatment.md.unit.overall_thermal_power_requirement() / feed_m3h(),
    )
    print(
        "MEC SEC (heat):",
        m.fs.treatment.costing.SEC_th_component[
            "fs.treatment.mec.unit.effects[1].effect"
        ](),
    )
    ...
    print(
        "VAGMD SEC (electric):",
        m.fs.treatment.md.unit.overall_elec_power_requirement() / feed_m3h(),
    )
    print(
        "EC SEC (electric):",
        m.fs.treatment.costing.SEC_elec_component["fs.treatment.EC.ec"](),
    )
