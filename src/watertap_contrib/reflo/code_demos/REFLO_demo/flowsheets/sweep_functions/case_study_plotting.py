...
def case_study_stacked_plot(
    ...
    for x, row in df.iterrows():
        ...
        try:
            row_lcow = row[f"{costing_blk}.LCOT"]
        except KeyError:
        ...
        for flow_label, flow_name in agg_flows.items():
            flow_lcow = 0
            try:
                print("REFLOSystem costing block aggregate flows", flow_label)
...
def plot_elec(df,flow_col,ax_dict):
    # Add electricity
    label_fontsize=16
    tick_fontsize=14

    fig, ax0 = plt.subplots()
    ax0.plot(df.index,df['fs.treatment.costing.aggregate_flow_electricity'],'-o',c='blue',label='Demand')
    ax0.set_ylabel("Aggregated Electricity Demand (kW)",fontsize=label_fontsize)
    ax0.set_ylim([0,round(max(df['fs.treatment.costing.aggregate_flow_electricity']),-3)])

    ax1 = ax0.twinx()
    ax1.plot(df.index,df['fs.treatment.costing.aggregate_flow_electricity']/df[flow_col]/3600,'-o',c='orange',label='SEC')
    ax1.set_ylim([0,10])
    ax1.set_ylabel("Specific Electricity Demand (kW/m$^3$)",fontsize=label_fontsize)

    ax0.set_xlim(df.index.min(), df.index.max())
    ax0.set_xlabel(ax_dict["xlabel"], fontsize=label_fontsize)
    ...
    ax0.legend(handles,labels, frameon=False)

    return fig


def plot_heat(df,flow_col,ax_dict):
    # Add electricity
    label_fontsize=16
    tick_fontsize=14

    fig, ax0 = plt.subplots()
    ax0.plot(df.index,df['fs.treatment.costing.aggregate_flow_heat'],'-o',c='blue',label='Demand')
    ax0.set_ylabel("Aggregated Heat Demand (kW)",fontsize=label_fontsize)
    ax0.set_ylim([0,round(max(df['fs.treatment.costing.aggregate_flow_heat']),-3)])

    ax1 = ax0.twinx()
    ax1.plot(df.index,df['fs.treatment.costing.aggregate_flow_heat']/df[flow_col]/3600,'-o',c='orange',label='SEC')
    ax1.set_ylim([0,100])
    ax1.set_ylabel("Specific Heat Demand (kW/m$^3$)",fontsize=label_fontsize)
