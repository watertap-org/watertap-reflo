...
def build_cryst(m, cryst_config):
    ...
    m.fs.treatment.costing.nacl_recovered.cost.set_value(
        cryst_config["nacl_recover_price"]
    )
...
def add_treatment_costing(m):
    ...
    heat_price = value(
        pyunits.convert(0.00894 * pyunits.USD_2023, to_units=pyunits.USD_2018)
    )
...
if __name__ == "__main__":
    ...
    print(
        "brine",
        value(
            m.fs.treatment.FO.fs.fo.brine_props[0].conc_mass_phase_comp["Liq", "TDS"]
        ),
    )
