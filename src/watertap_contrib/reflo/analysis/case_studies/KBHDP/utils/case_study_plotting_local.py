...
cases = {
    "KBHDP": {
        "KBHDP_SOA_1": {
            ...
            "water_recovery": {
                "file": os.path.join(
                    # sweep_csv_dir, "sweep_data_analysisType_KBHDP_SOA_1_water_recovery.csv"
                    sweep_csv_dir, "sweep_data_KBHDP_SOA_1_soda_ash_cost.csv"
                ),
                "global_costing_blk": "fs.treatment.costing",
                "costing_blk": "fs.treatment.costing",
                "unit_dict": {
                ...
                "flow_col": "fs.treatment.product.properties[0.0].flow_vol_phase[Liq]",
                "ax_dict": dict(xlabel="Soda Ash Cost ($/kg)", ylabel="LCOW (\$/m$^3$)"),
                "ylims": (0, 3),
                "xlims": (0.12, 0.36),
    ...
    "Permian": {
        "Permian_RPT1_MD": {
            ...
            #     "flow_col": "fs.treatment.product.properties[0.0].flow_vol_phase[Liq]",
            #     "ax_dict": dict(xlabel="Renewable Energy (%)", ylabel="LCOW (\$/m$^3$)"),
            #     "ylims": (0, 40),
        ...
        "Permian_RPT1_FO": {
            "water_recovery": {
                "file": os.path.join(
                    ...
                    sweep_csv_dir, "permian_RPT2_FO_DWI_no_CST_sweep_recovery_ratio.csv"
                ),
                "global_costing_blk": "fs.treatment.costing",
                "costing_blk": "fs.treatment.costing",
                "unit_dict": {
    ...
    for idx0, study in enumerate(cases):
        ...
        for key in cases[study][list(cases[study].keys())[0]].keys():
            ...
            figs[study][key], axs[study][key] = plt.subplots(
                n_rows, n_cols, figsize=(4.0 * n_cols, 4 * n_rows),
    ...
    for idx0, study in enumerate(cases):
        for key in cases[study][list(cases[study].keys())[0]].keys():
            ...
            for idx1, case in enumerate(cases[study]):
                ...
                axs[study][key][idx1].text(-0.25, 0.95, f'{fig_letters[idx1]})', fontsize=20, ha='center', va='center', transform=axs[study][key][idx1].transAxes,)
            ...
            bb = master_legend.get_bbox_to_anchor().transformed(axs[study][key][idx1].transAxes.inverted()) 

            # Change to location of the legend. 
            yOffset = 0.025
            bb.y0 += yOffset
            bb.y1 += yOffset
            master_legend.set_bbox_to_anchor(bb, transform = axs[study][key][idx1].transAxes)
