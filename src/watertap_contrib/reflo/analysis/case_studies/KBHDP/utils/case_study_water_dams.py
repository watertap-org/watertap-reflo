...
def save_water_dams(
    ...
    water_dams_df["Treatment Units Heat Consumed (MWh/year)"] = (
        df["fs.treatment.costing.aggregate_flow_heat"]
    ...
    water_dams_df["Treatment Units Electricity Consumed (MWh/year)"] = (
        df["fs.treatment.costing.aggregate_flow_electricity"]
        * pyunits.convert(1 * pyunits.kW, to_units=pyunits.MWh / pyunits.year)()
    )
    ...
    water_dams_df.insert(
        1, "Total Capex ($)", total_treatment_capex + total_energy_capex
    )
    water_dams_df.insert(
        2, "Total Opex ($/year)", total_treatment_opex + total_energy_opex
    )
    ...
    if feed_conditions != None:
        water_dams_df.insert(
            3,
            "Feed Flow Rate (kg/s)",
            df[feed_conditions["feed_flow_mass_water"]]
            + df[feed_conditions["feed_flow_mass_tds"]],
        )
        water_dams_df.insert(
            4, "TDS Flow Rate (kg/s)", df[feed_conditions["feed_flow_mass_tds"]]
        )
    else:
