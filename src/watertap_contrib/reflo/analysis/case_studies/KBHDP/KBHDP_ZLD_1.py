...
from components.lsrro_system import (
    build_ro,
...
from components.EC import (
    build_ec,
    init_ec,
    report_EC,
    set_ec_operating_conditions,
    # set_ec_op_conditions,
)
...
def build_system():
    ...
    build_lsrro(m, m.fs.LSRRO, prop_package=m.fs.RO_properties)
...
def add_connections(m):
    ...
    m.fs.lsrro_to_disposal = Arc(
        source=m.fs.LSRRO.disposal.outlet,
...
def set_operating_conditions(m):
    ...
    set_ro_system_operating_conditions(
        m, m.fs.LSRRO, mem_area=10000, RO_pump_pressure=20e5
    )
