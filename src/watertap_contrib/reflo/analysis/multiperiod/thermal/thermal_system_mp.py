...
def get_variable_pairs_steady_treatment(t1, t2):
    ...
    return [
        (
            t1.fs.hx_solar.hot_side_outlet.temperature[0],
            t2.fs.previous_hx_solar_hot_outlet_temperature,
        ),
        ...
        (
            t1.fs.hx_solar.cold_side_outlet.temperature[0],
            t2.fs.previous_hx_solar_cold_outlet_temperature,
        ),
...
def get_variable_pairs(t1, t2):
    ...
    return [
        ...
        (
            t1.fs.tes.tes_process_outlet.temperature[0],
            t2.fs.previous_process_outlet_temperature,
        ),
        (t1.fs.acc_grid_duty, t2.fs.previous_acc_grid_duty),
        (t1.fs.grid_heater.heat_duty[0], t2.fs.previous_grid_duty),
    ]
...
def create_multiperiod_thermal_model(
    ...
    active_blocks[0].fs.previous_hx_solar_hot_outlet_temperature.fix(
        initial_temperature["solar_hx_hot_outlet"] + 273.15
    )
    active_blocks[0].fs.previous_fpc_outlet_temperature.fix(
        initial_temperature["fpc_outlet"] + 273.15
    )
    active_blocks[0].fs.previous_tes_tank_temp.fix(
        initial_temperature["tes_tank"] + 273.15
    )
    active_blocks[0].fs.previous_hx_solar_cold_outlet_temperature.fix(
        initial_temperature["solar_hx_cold_outlet"] + 273.15
    )

    if process_outlet_temperature == None:
        active_blocks[0].fs.previous_process_outlet_temperature.fix(
            initial_temperature["process_outlet"] + 273.15
        )
...
if __name__ == "__main__":
    ...
    for n in range(0, n_time_points):
        ...
        process_outlet.append(
            mp.blocks[n].process.fs.tes.tes_process_inlet.temperature[0]() - 273.15
        )
        grid_heater_outlet.append(
            mp.blocks[n].process.fs.grid_heater.outlet.temperature[0]() - 273.15
        )

        fpc_inlet.append(
            mp.blocks[n].process.fs.fpc.inlet.temperature[0].value - 273.15
        )
        fpc_outlet.append(mp.blocks[n].process.fs.fpc.outlet.temperature[0]() - 273.15)

        hx_hot_inlet.append(
            mp.blocks[n].process.fs.hx_solar.hot_side_inlet.temperature[0].value
            - 273.15
        )
        hx_hot_outlet.append(
            mp.blocks[n].process.fs.hx_solar.hot_side_outlet.temperature[0].value
            - 273.15
        )

        hx_cold_inlet.append(
            mp.blocks[n].process.fs.hx_solar.cold_side_inlet.temperature[0].value
            - 273.15
        )
        hx_cold_outlet.append(
            mp.blocks[n].process.fs.hx_solar.cold_side_outlet.temperature[0].value
            - 273.15
        )
    ...
    ax1.plot(
        range(0, n_time_points), hx_hot_inlet, label="Solar HX hot inlet", marker="o"
    )
    ax1.plot(
        range(0, n_time_points), hx_hot_outlet, label="Solar HX hot outlet", marker="o"
    )

    ax1.plot(
        range(0, n_time_points), hx_cold_inlet, label="Solar HX cold inlet", marker="o"
    )
    ax1.plot(
        range(0, n_time_points),
        hx_cold_outlet,
        label="Solar HX cold outlet",
        marker="o",
    )

    ax2.plot(range(0, n_time_points), tes_temp, label="TES temp", marker="o")
    ax2.plot(
        range(0, n_time_points), process_outlet, label="Process to TES", marker="o"
    )
    ax2.plot(
        range(0, n_time_points),
        grid_heater_outlet,
        label="Grid heater outlet",
        marker="o",
    )
    ...
    plt.subplots_adjust(
        left=0.1, bottom=0.05, right=0.9, top=0.9, wspace=0.2, hspace=0.5
    )
    plt.show()
