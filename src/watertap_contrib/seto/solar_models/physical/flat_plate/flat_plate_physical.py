...
class FlatPlatePhysicalData(SolarEnergyBaseData):
    ...
    def build(self):
        ...
        # ==========PARAMETERS==========
        self.phi = Param(
            initialize=1, units=pyunits.degrees, doc="Latitude of collector"
        )

        self.lon = Param(
            initialize=1, units=pyunits.degrees, doc="Longitude of collector"
        )

        self.std_meridian = Param(
            initialize=1, units=pyunits.degrees, doc="Standard meridian of collector"
        )
        ...
        self.beta = Param(
            initialize=1, units=pyunits.degrees, doc="Tilt angle of collector"
        )
        ...
        @self.Constraint(
            doc="Ratio of beam radiation on tilted surface to horizontal surface in northern hemisphere for collectors facing south, D&B 4th ed. eqn. 1.8.2"
        )
        def eq_R_b(b):
            return self.R_b == (
                (
        ...
        @self.Constraint(
            doc="Total radiation on tilted surface via isotropic diffuse sky model"
        )
        def eq_G_T(b):
            return self.G_T == (
                self.G_b * self.R_b
