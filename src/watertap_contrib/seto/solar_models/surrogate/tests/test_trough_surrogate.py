...
from watertap_contrib.seto.solar_models.surrogate.training_surrogate import create_rbf_surrogate
...
EXPECTED_HEAT_ENERGIES = [7.997e8, 5.259e8, 5.753e8, 6.244e8, 6.719e8, 7.169e8, 7.585e8, 7.958e8, 8.285e8, 8.563e8]
EXPECTED_ELECTRICITY_USE = [1.534e7, 4.599e6, 4.014e6, 3.990e6, 4.351e6, 4.924e6, 5.560e6, 6.150e6, 6.630e6, 6.983e6]
...
class TestTrough:
    ...
    def test_training(self, data):
        ...
        EXPECTED_HEAT_ENERGIES_TEST = [8.034e8, 3.505e8, 3.668e8, 3.814e8, 3.946e8, 4.065e8, 4.175e8, 4.275e8, 4.366e8, 4.445e8]
        EXPECTED_ELECTRICITY_USE_TEST = [7.176e6, 1.301e6, 1.302e6, 1.303e6, 1.304e6, 1.305e6, 1.306e6, 1.307e6, 1.308e6, 1.308e6]
        ...
        test_surrogate = create_rbf_surrogate(data['training'], INPUT_LABELS, OUTPUT_LABELS, test_surrogate_filename)
        ...
        assert list(output['annual_energy']) == pytest.approx(EXPECTED_HEAT_ENERGIES_TEST, 1e-3)
        assert list(output['electrical_load']) == pytest.approx(EXPECTED_ELECTRICITY_USE_TEST, 1e-3)
    ...
    def test_loading(self, data):
        ...
        assert list(output['annual_energy']) == pytest.approx(EXPECTED_HEAT_ENERGIES, 1e-3)
        assert list(output['electrical_load']) == pytest.approx(EXPECTED_ELECTRICITY_USE, 1e-3)
