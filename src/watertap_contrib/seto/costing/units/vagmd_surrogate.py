...
def cost_vagmd_surrogate(blk):
    ...
    n_time_points = len(vagmd.mp.get_active_process_blocks())

    blk.num_modules_constraint = pyo.Constraint(
        expr=blk.num_modules
        == pyo.units.convert(
            vagmd.system_capacity
            / sum(
                vagmd.mp.get_active_process_blocks()[i].fs.vagmd.permeate_flux
                for i in range(n_time_points)
            )
            * n_time_points
            / vagmd.mp.get_active_process_blocks()[0].fs.vagmd.module_area,
    ...
    blk.module_cost_constraint = pyo.Constraint(
        ...
        * (blk.num_modules / vagmd_params.base_module_capacity)
        ** vagmd_params.module_cost_index
        + vagmd_params.membrane_cost
        * vagmd.mp.get_active_process_blocks()[0].fs.vagmd.module_area
        * blk.num_modules
    )
    ...
    blk.other_capital_cost_constraint = pyo.Constraint(
        ...
        * (blk.num_modules / vagmd_params.base_housing_rack_capacity)
        ** vagmd_params.housing_rack_cost_index
        + vagmd_params.base_tank_cost
        * (blk.num_modules / vagmd_params.base_tank_capacity)
        ** vagmd_params.tank_cost_index
        + vagmd_params.base_other_cost
        * (blk.num_modules / vagmd_params.base_other_capacity)
    ...
    if iscale.get_scaling_factor(blk.num_modules) is None:
        iscale.set_scaling_factor(blk.num_modules, 1e-3)
    iscale.constraint_scaling_transform(blk.num_modules_constraint, 1e-3)

    if iscale.get_scaling_factor(blk.module_cost) is None:
        iscale.set_scaling_factor(blk.module_cost, 1e-5)
    iscale.constraint_scaling_transform(blk.module_cost_constraint, 1e-5)

    if iscale.get_scaling_factor(blk.other_capital_cost) is None:
        iscale.set_scaling_factor(blk.other_capital_cost, 1e-5)
    iscale.constraint_scaling_transform(blk.other_capital_cost_constraint, 1e-5)

    if iscale.get_scaling_factor(blk.capital_cost) is None:
        iscale.set_scaling_factor(blk.capital_cost, 1e-5)
    iscale.constraint_scaling_transform(blk.capital_cost_constraint, 1e-5)
