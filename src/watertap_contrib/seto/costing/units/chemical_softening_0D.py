...
def cost_chem_softening(blk,softeningType = SofteningProcedureType.single_stage_lime):
    '''
        ...
        1. Lime softening
    '''
    softeningType = blk.unit_model.config.softening_procedure_type

    if (softeningType == SofteningProcedureType.single_stage_lime):
        cost_chem_softening_single_stage_lime(blk)
    # elif (softeningType == SofteningProcedureType.excess_lime):
    #     cost_chem_softening_excess_lime(blk)
    # elif (softeningType == SofteningProcedureType.single_stage_lime_soda):
    #     cost_chem_softening_single_stage_lime_soda(blk)
    else:
        cost_chem_softening_excess_lime_soda(blk)


def cost_chem_softening_single_stage_lime(blk):
    '''
        Capital and operating costs for single stage lime softening
    '''
    ...
    blk.recarb_basin_source_capital_cost_constraint = Constraint(
        ...
        ==  (9e-8 * (pyunits.convert(blk.unit_model.CO2_first_basin ,to_units = pyunits.lb / pyunits.day)) - 
            0.001 * (pyunits.convert(blk.unit_model.CO2_first_basin,to_units = pyunits.lb / pyunits.day)) ** 2 + 
            42.578 * (pyunits.convert(blk.unit_model.CO2_first_basin ,to_units = pyunits.lb / pyunits.day)) + 130812 )
        )
    ...
    blk.recarb_basin_op_cost_constraint = Constraint(
        ...
        ==  ((1e-8 * (pyunits.convert(blk.unit_model.CO2_first_basin,to_units = pyunits.lb / pyunits.day))**3) - 
            (0.0004 * (pyunits.convert(blk.unit_model.CO2_first_basin,to_units = pyunits.lb / pyunits.day)) ** 2) + 
            (6.19 * (pyunits.convert(blk.unit_model.CO2_first_basin ,to_units = pyunits.lb / pyunits.day)) + 10265))
        ) 
    ...
    if blk.unit_model.config.silica_removal:
        blk.mg_add_op_cost_constraint = Constraint(
            ...
            == (pyunits.convert(blk.unit_model.MgCl2_dosing*blk.unit_model.properties_in[0].flow_vol, to_units = pyunits.kg/pyunits.year)*chem_soft.MgCl2_cost)
            )
    ...
    if blk.unit_model.config.silica_removal:
        blk.fixed_operating_cost_constraint = Constraint(
            ...
            blk.lime_feed_op_cost + blk.mg_add_op_cost + blk.lime_sludge_mngt_op_cost + blk.admin_op_cost
        )
    else:
        blk.fixed_operating_cost_constraint = Constraint(
            expr = blk.fixed_operating_cost
            ==  blk.mix_tank_op_cost + blk.floc_tank_op_cost + blk.sed_basin_op_cost + blk.recarb_basin_op_cost +
            blk.lime_feed_op_cost + blk.lime_sludge_mngt_op_cost + blk.admin_op_cost
        )
...
def cost_chem_softening_excess_lime_soda(blk):
    '''
        Capital and operating costs for excess lime and soda softening
    '''
    ...
    if blk.unit_model.config.silica_removal:
        ...
        blk.fixed_operating_cost_constraint = Constraint(
            expr = blk.fixed_operating_cost
            ==  blk.mix_tank_op_cost + blk.floc_tank_op_cost + blk.sed_basin_op_cost + blk.recarb_basin_op_cost +
            blk.lime_feed_op_cost + blk.soda_ash_add_op_cost + blk.lime_sludge_mngt_op_cost + blk.admin_op_cost
        )
