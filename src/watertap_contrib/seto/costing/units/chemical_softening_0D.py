...
def cost_chem_softening(blk):
    ...
    if value(blk.unit_model.vel_gradient_mix) <= 300:
        blk.mix_tank_capital_cost_constraint = Constraint(
            ...
            == (
                ...
                * (
                    pyunits.convert(
                        blk.unit_model.volume_mixer, to_units=pyunits.ft**3
                    )
                )
                ** 2
            )
        ...
        value(blk.unit_model.vel_gradient_mix) > 300
        and value(blk.unit_model.vel_gradient_mix) <= 600
    ):
        blk.mix_tank_capital_cost_constraint = Constraint(
            expr=blk.mix_tank_capital_cost
            == (
        ...
        value(blk.unit_model.vel_gradient_mix) > 600
        and value(blk.unit_model.vel_gradient_mix) <= 1000
    ):
        blk.mix_tank_capital_cost_constraint = Constraint(
            expr=blk.mix_tank_capital_cost
            == (
    ...
    if value(blk.unit_model.vel_gradient_floc) <= 20:
        ...
        value(blk.unit_model.vel_gradient_floc) > 20
        and value(blk.unit_model.vel_gradient_floc) <= 50
    ):
        blk.floc_tank_capital_cost_constraint = Constraint(
            expr=blk.floc_tank_capital_cost
        ...
        value(blk.unit_model.vel_gradient_floc) > 50
        and value(blk.unit_model.vel_gradient_floc) <= 80
    ):
        blk.floc_tank_capital_cost_constraint = Constraint(
            expr=blk.floc_tank_capital_cost
    ...
    blk.sed_basin_capital_cost_constraint = Constraint(
        ...
        == (
            ...
            ** 2
            + 86.89
            * (blk.unit_model.volume_sed * 35.315)
            / pyunits.convert(chem_soft.sed_basin_depth, to_units=pyunits.ft)
            + 182801
        )
    )
    ...
    blk.lime_feed_system_capital_cost_constraint = Constraint(
        ...
        == (
            ...
            * (
                pyunits.convert(
                    blk.unit_model.CaOH_dosing, to_units=pyunits.lb / pyunits.hour
                )
            )
            + 193268
        )
    ...
    blk.lime_feed_op_cost_constraint = Constraint(
        ...
        == (
            ...
            * (
                pyunits.convert(
                    blk.unit_model.CaOH_dosing, to_units=pyunits.lb / pyunits.day
                )
            )
            ** 0.4589
        )
    ...
    blk.soda_ash_add_op_cost_constraint = Constraint(
        ...
        == (
            pyunits.convert(
                blk.unit_model.Na2CO3_dosing, to_units=pyunits.kg / pyunits.year
            )
            * chem_soft.Na2CO3_cost
        )
    )
    ...
    if blk.unit_model.config.silica_removal:
        blk.mg_add_op_cost_constraint = Constraint(
            ...
            == (
                pyunits.convert(
                    blk.unit_model.MgCl2_dosing
                    * blk.unit_model.properties_in[0].flow_vol,
                    to_units=pyunits.kg / pyunits.year,
                )
                * chem_soft.MgCl2_cost
            )
        )
    ...
    blk.mixer_power_constraint = Constraint(
        expr=blk.mixer_power == blk.mixer_energy * 0.001 * 24
    )

    blk.floc_power_constraint = Constraint(
        expr=blk.floc_power == blk.floc_energy * 0.001 * 24
    )

    blk.electricity_flow_constraint = Constraint(
        expr=blk.electricity_flow == (blk.mixer_power + blk.floc_power)
    )
