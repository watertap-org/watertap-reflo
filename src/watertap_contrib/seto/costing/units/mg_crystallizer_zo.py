...
def cost_mg_crystallizer(blk):
    ...
    feed_water_flow_rate = pyo.units.convert(feed.flow_vol_phase["Liq"],
                                             to_units=pyo.units.m**3/pyo.units.day)
    
    crystallizer_capital_cost = pyo.Expr_if(feed_water_flow_rate <= 20,
    ...
    blk.fixed_operating_cost_constraint = pyo.Constraint(
        expr=blk.fixed_operating_cost == mg_cryst_params.unit_operating_cost 
                                         * pyo.units.convert(feed.flow_vol_phase["Liq"],
                                                             to_units=pyo.units.m**3/pyo.units.year)
                                         + mg_cryst_params.calcium_hydroxide_dosing_cost
                                         * pyo.units.convert(mg_cryst.calcium_hydroxide_mass,
                                                             to_units=pyo.units.kg/pyo.units.year)
                                         - blk.include_metal_recovery 
                                         * (mg_cryst_params.hydromagnesite_recovery_revenue
                                            * pyo.units.convert(mg_cryst.hydromagnesite_precipitated, to_units = pyo.units.kg/pyo.units.year)
                                            + mg_cryst_params.calcite_recovery_revenue
                                            * pyo.units.convert(mg_cryst.calcite_precipitated_feed + mg_cryst.calcite_precipitated_dose,
                                                                to_units=pyo.units.kg/pyo.units.year))
    )
