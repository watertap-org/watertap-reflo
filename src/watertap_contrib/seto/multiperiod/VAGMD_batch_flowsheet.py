...
def build_vagmd_flowsheet(
    ...
    """    
    @m.Constraint(doc="Calculate time interval of each period")
    def eq_dt(b):
        if module_type == "AS7C1.5L":
            return b.fs.vagmd.dt == 20352.55 / feed_flow_rate
        else: # module_type == "AS26C7.2L"
            return b.fs.vagmd.dt == 73269.19 / feed_flow_rate

    @m.Constraint(doc="Calculate current feed salinity")
    def eq_feed_salinity(b):
        return (
        b.fs.vagmd.feed_props[0].conc_mass_phase_comp["Liq", "TDS"] * 
        (initial_batch_volume - b.fs.acc_distillate_volume) 
        == b.fs.pre_feed_salinity *
        (initial_batch_volume - b.fs.pre_acc_distillate_volume) 
        )   

    @m.Constraint(doc="Calculate current feed temperature")
    def eq_feed_temp(b):
        return (b.fs.vagmd.feed_props[0].temperature 
                == (feed_flow_rate * pyunits.convert(b.fs.vagmd.dt, to_units = pyunits.h)
                * b.fs.pre_evap_out_temp
                + (initial_batch_volume - b.fs.pre_acc_distillate_volume)
                * b.fs.pre_feed_temperature)
                / (feed_flow_rate * pyunits.convert(b.fs.vagmd.dt, to_units = pyunits.h)
                + initial_batch_volume
                - b.fs.pre_acc_distillate_volume)
        )

    @m.Constraint(doc="Calculate accumulated distillate volume")
    def eq_acc_distillate_volume(b):
        return (
        b.fs.acc_distillate_volume == b.fs.pre_acc_distillate_volume 
                                    + pyunits.convert(b.fs.pre_permeate_flow_rate * b.fs.vagmd.dt,
                                                    to_units = pyunits.L)
        )    
    
    @m.Constraint(doc="Calculate accmulated recovery ratio")
    def eq_acc_recovery_ratio(b):
        return (
                    b.fs.acc_recovery_ratio
                    == 1
                    - feed_salinity
                    / b.fs.vagmd.feed_props[0].conc_mass_phase_comp["Liq", "TDS"]
                )
    
    @m.Constraint(doc="Calculate accmulated thermal energy consumption")
    def eq_acc_thermal_energy(b):
        return (b.fs.acc_thermal_energy == b.fs.pre_acc_thermal_energy + b.fs.vagmd.thermal_energy)
