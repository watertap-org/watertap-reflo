...
class TestMgCrystallizer:
    ...
    def test_build(self, Mg_crystallizer_frame):
        ...
        assert number_unused_variables(m) == 87  # vars from property package parameters
    ...
    def test_solution(self, Mg_crystallizer_frame):
        ...
        assert (
            pytest.approx(78.128, rel=1e-3)
            == results["NaCl concentration in the waste flow (g/L)"]
        )
        assert (
            pytest.approx(605.388, rel=1e-3)
            == results["Hydromagnesite precipitated (kg/day)"]
        )
        assert (
            pytest.approx(907.358, rel=1e-3) == results["Calcite precipitated (kg/day)"]
        )
        ...
        assert pytest.approx(3.430, rel=1e-3) == results["Suspension Volume (m3)"]
        assert pytest.approx(2.25, rel=1e-3) == results["Crystallizer height (m)"]
    ...
    def test_costing(self, Mg_crystallizer_frame):
        ...
        m.fs.costing.add_annual_water_production(
            mg_cryst.properties_in[0].flow_vol_phase["Liq"]
        )
        ...
        assert pytest.approx(16912.904, rel=1e-3) == value(
        ...
        assert pytest.approx(-226211.071, rel=1e-3) == value(
            m.fs.costing.total_operating_cost
        )
        assert pytest.approx(20160, rel=1e-3) == value(m.fs.costing.total_capital_cost)
