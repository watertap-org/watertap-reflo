    ...
    for key, val in data_dict.items():
        ...
        if val["results"] != np.nan:
            try:
                ...
                grid[val["idx1"], val["idx2"]] = getattr(
                    val["model"].fs.sys_costing, metric
                )()
            except:
                print("fail")
                z.append(np.nan)
...
def contour_LCOW(
    ...
    **kwargs,
    ...
    if levels == None:
        levels = np.arange(
            round(low, 1), round(high, 1), (round(high, 1) - round(low, 1)) / 4
        )
    ...
    ax.clabel(
        cs3, fmt="$%1.2f ", colors="k", fontsize=12, inline_spacing=contour_label_space
    )
