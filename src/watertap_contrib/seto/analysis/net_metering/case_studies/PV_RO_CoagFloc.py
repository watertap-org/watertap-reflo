...
def coag_floc_costing(m):
    ...
    m.fs.coag_and_floc.costing = UnitModelCostingBlock(
        flowsheet_costing_block=m.fs.coag_floc_costing
    )
    ...
    m.fs.coag_and_floc.costing.LCOW_constraint = Constraint(
        ...
        == (
            (
                pyunits.convert(
                    m.fs.coag_and_floc.costing.capital_cost, to_units=pyunits.USD_2021
                )
                / m.fs.coag_floc_costing.plant_lifetime
            )
            / pyunits.convert(
                m.fs.coag_and_floc.properties[0.0].flow_vol,
        ...
        for idx2, rec_val in enumerate(rec_vals):
            try:
                ...
                try:
                    ...
                    solve_report.append(
                        [
                            f"Optimal solve for {Q_basis} MGD, {conc_val} g/L, {rec_val} recovery: LCOW={lcow}"
                        ]
                    )
                except:
                    ...
                    solve_report.append(
                        [
                            f"Infeasible solve for {Q_basis} MGD, {conc_val} g/L, {rec_val} recovery"
                        ]
                    )
                    # continue
                ...
                solve_report.append(
                    [
                        f"Failed Initialization for {Q_basis} MGD, {conc_val} g/L, {rec_val} recovery"
                    ]
                )

            temp_dict["Scenario {}".format(count)] = {
                "idx1": idx1,
                "idx2": idx2,
                "var1": conc_val,
                "var2": rec_val,
