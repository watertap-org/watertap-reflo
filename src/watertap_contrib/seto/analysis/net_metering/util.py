...
def display_pv_results(m, sep="."):
    ...
    print(f'{"LCOE":<34s}{f"${(m.fs.sys_costing.LCOE()):<25.3f}"}{"$/kWh":<10s}')
    print("\n")
...
def display_costing_breakdown(m, sep="."):
    ...
    for f in m.fs.energy.costing.used_flows:
        print(
            f'{f"En. Flow [{f}]":<35s}{value(getattr(m.fs.energy.costing, f"aggregate_flow_{f}")):<25.1f}{"kW":<10s}'
        )
    for f in m.fs.treatment.costing.used_flows:
        print(
            f'{f"Treat Flow [{f}]":<35s}{value(getattr(m.fs.treatment.costing, f"aggregate_flow_{f}")):<25.1f}{"kW":<10s}'
        )

    print(
        f'{f"System Flow [electricity]":<35s}{value(m.fs.sys_costing.aggregate_flow_electricity):<25.1f}{"kW":<10s}'
    )

    print(
        f'{f"Treat Total [{f}]":<35s}{pyunits.convert(m.fs.treatment.costing.aggregate_flow_electricity, to_units=pyunits.kWh/pyunits.year)()/1000:<25,.0f}{"MWh/yr":<10s}'
    )
    print(
        f'{f"System Total [{f}]":<35s}{pyunits.convert(m.fs.sys_costing.aggregate_flow_electricity, to_units=pyunits.kWh/pyunits.year)()/1000:<25,.0f}{"MWh/yr":<10s}'
    )
    print(
        f'{"Energy Op Cost Electric":<34s}{f"${m.fs.energy.costing.total_electric_operating_cost():<25,.0f}"}{"$/yr":<10s}'
    )
    print(
        f'{"Treat Op Cost Electric":<34s}{f"${m.fs.treatment.costing.total_electric_operating_cost():<25,.0f}"}{"$/yr":<10s}'
    )
    print(
        f'{"System Op Cost Electric":<34s}{f"${m.fs.sys_costing.total_electric_operating_cost():<25,.0f}"}{"$/yr":<10s}'
    )
    print(
        f'{"System Agg Flow Electric":<35s}{f"{m.fs.sys_costing.aggregate_flow_electricity():<25.2f}"}{"kW":<10s}'
    )
    print(
        f'{"System Net Electric":<35s}{f"{m.fs.sys_costing.net_flow_electricity():<25.2f}"}{"kW":<10s}'
    )
    print(
        f'{"System Net Electric":<35s}{f"{pyunits.convert(m.fs.sys_costing.net_flow_electricity, to_units=pyunits.kWh/pyunits.year)()/1000:<25.2f}"}{"MWh/yr":<10s}'
    )
    print(
        f'{"System Agg Electric":<35s}{f"{pyunits.convert(m.fs.sys_costing.aggregate_flow_electricity, to_units=pyunits.kWh/pyunits.year)()/1000:<25.2f}"}{"MWh/yr":<10s}'
    )
    print(
        f'{"Net Electric Cost":<34s}{f"${m.fs.sys_costing.net_flow_electricity_cost():<25,.0f}"}{"$/yr":<10s}'
    )
    print(
        f'{"Total Op Cost":<34s}{f"${m.fs.sys_costing.total_operating_cost():<25,.0f}"}{"$/yr":<10s}'
    )
