...
from idaes.models.unit_models import Product, Feed, Mixer, Separator, MomentumMixingType, MixingType
...
from watertap_contrib.seto.unit_models.surrogate import VAGMDSurrogateBase, LTMEDSurrogate
...
def build_med_md_flowsheet(
    ...
    med_inputs = {'feed_salinity': med_feed_salinity,
    ...
    m.fs.S1 = Separator(property_package =m.fs.liquid_prop,
    ...
    m.fs.M1 = Mixer(property_package=m.fs.liquid_prop,
                       material_balance_type=MaterialBalanceType.componentPhase,
                       energy_mixing_type=1,
                       inlet_list = ['MED_brine','remained_liquid', 'MD_brine'],
                       )
    
    vagmd_inputs = {'feed_flow_rate': md_feed_flow_rate,
                    'evap_inlet_temp': 80,
                    'cond_inlet_temp': 25,
                    'feed_temp': 25,
    ...
    m.fs.S2 = Separator(property_package =m.fs.liquid_prop,
    ...
    m.fs.separator_to_mixer = Arc(
        source=m.fs.S1.separator_to_mixer, 
        destination=m.fs.M1.MED_brine
    )   

    m.fs.vagmd_to_mixer = Arc(
        source=m.fs.vagmd.brine, 
        destination=m.fs.M1.MD_brine
    )   
...
def add_processing_phase_constraint(mfs,  phase,  batch_volume, dt = None,): 
    ...
    if phase == 'processing':
        ...
        def eq_dt(b):
            if dt:
                ...
                return b.dt == 20352.55 / pyunits.convert(mfs.vagmd.feed_props[0].flow_vol_phase["Liq"],
                                                        to_units= pyunits.L / pyunits.h)
            else:  # module_type == "AS26C7.2L"
                return b.dt == 73269.19 / pyunits.convert(mfs.vagmd.feed_props[0].flow_vol_phase["Liq"],
                                                        to_units= pyunits.L / pyunits.h)
        
        @mfs.Constraint(doc="remained volume is the batch volume")
        def eq_S2_remained_volume(b):
            return (b.S2.remained_liquid_state[0].flow_vol_phase["Liq"] * b.dt
                    == pyunits.convert(batch_volume * pyunits.L,
        ...
        def eq_tank_volume(b):
            return (b.volume_in_tank == b.volume_in_tank_previous +
                    pyunits.convert(b.S1.overflow_state[0].flow_vol_phase["Liq"] * b.dt,
                                    to_units = pyunits.m**3)
                    )
...
def add_med(fs, inputs):
    ...
    sf = 1 / 1000 / pyunits.convert_value(inputs['sys_capacity'], 
    ...
    fs.vapor_prop.set_default_scaling(
        "flow_mass_phase_comp", 1, index=("Liq", "H2O")
    )
    fs.vapor_prop.set_default_scaling(
        "flow_mass_phase_comp", 1e2, index=("Vap", "H2O")
    )
...
def fix_dof_and_initialize(
    ...
    calculate_variable_from_constraint(m.fs.dt,
                                       m.fs.eq_dt)
