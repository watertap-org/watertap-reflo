...
class TestVAGMDbatch:
    ...
    def VAGMD_batch_frame(self):
        ...
        m.fs.VAGMD.system_capacity.fix(1000)  # Fix system capacity, m3/day
    ...
    def test_solution(self, VAGMD_batch_frame):
        m = VAGMD_batch_frame
        vagmd = m.fs.VAGMD
        overall_performance, data_table = vagmd.get_model_performance()
        ...
        assert value(vagmd.thermal_power_requirement) == pytest.approx(
            7880.753, abs=1e-3
        )
        assert value(vagmd.elec_power_requirement) == pytest.approx(14.958, abs=1e-3)
    ...
    def test_costing(self, VAGMD_batch_frame):
        ...
        assert pytest.approx(1109.92, rel=1e-3) == value(vagmd.costing.num_modules)
        assert pytest.approx(1515161.752, rel=1e-3) == value(vagmd.costing.module_cost)
