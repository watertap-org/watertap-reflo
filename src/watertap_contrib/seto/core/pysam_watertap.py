...
class PySAMWaterTAP:
    ...
    def __init__(self, pysam_model=None, tech_config_file=None, grid_config_file=None, 
        rate_config_file=None, cash_config_file=None, weather_file=None, dcac_ratio=1.2):
        ...
        self._config_files = [self._tech_config_file, self._grid_config_file, self._rate_config_file, self._cash_config_file]
    ...
    def setup_pv_single_owner(self):
        ...
        self.rate_model = utilityrate.from_existing(self.tech_model, self._pysam_model_name)
        self.cash_model = singleowner.from_existing(self.tech_model, self._pysam_model_name)
        self._modules = [self.tech_model, self.grid_model, self.rate_model, self.cash_model]
        self._load_config_files()
        self.tech_model.SolarResource.solar_resource_file = self._weather_file
    

    def run_pv_single_owner(self, desired_size=50, desired_dcac_ratio=1.2, tech_model_kwargs={}, cash_model_kwargs={}):
        self._size_pv_array(desired_size=desired_size, desired_dcac_ratio=desired_dcac_ratio)
    ...
    def _size_pv_array(self, desired_size=50, desired_dcac_ratio=1.2):
        ...
        m['spe_imp'] = self._spe_power(m['spe_eff4'], m['spe_rad4'], m['spe_area']) / m['spe_vmp']    # 4 = reference conditions
        mod_imp = [m['spe_imp'], m['cec_i_mp_ref'], m['sixpar_imp'], m['snl_impo'], m['sd11par_Imp0']][module_model]
        mod_vmp = [m['spe_vmp'], m['cec_v_mp_ref'], m['sixpar_vmp'], m['snl_vmpo'], m['sd11par_Vmp0']][module_model]
        mod_voc = [m['spe_voc'], m['cec_v_oc_ref'], m['sixpar_voc'], m['snl_voco'], m['sd11par_Voc0']][module_model]
        ...
        inv_vdcmax = [m['inv_snl_vdcmax'], m['inv_ds_vdcmax'], m['inv_pd_vdcmax'], m['inv_cec_cg_vdcmax']][inverter_model]          # Vdcmax
        inv_power = [m['inv_snl_paco'], m['inv_ds_paco'], m['inv_pd_paco'], m['inv_cec_cg_paco']][inverter_model]                   # Paco
        inv_dc_power = [m['inv_snl_pdco'], m['inv_ds_pdco'], m['inv_pd_pdco'], m['inv_cec_cg_pdco']][inverter_model]                # Pdco
        ...
        if self.desired_dcac_ratio > 0:
            inverters = ((num_series * num_parallel * mod_power)) / (self.desired_dcac_ratio * inv_power)
