...
The following models are available in REFLO for simulating solar energy systems.
...
Every surrogate solar model in REFLO has at least one required input variable (conventionally the system capacity) but may have many optional
input variables. The optional input variables are typically design parameters that the user can choose to vary
when generating the training data for the surrogate. 

For example, the :ref:`FPC surrogate model <fpc_surrogate_ref>` has three design variables:
...
or they can choose to hold one or more of these parameters constant. The number of input variables that are varied when generating the training data
...
1. The user chooses to vary only the system capacity when generating the training data and uses 12 hours of storage and 60°C for the hot temperature set point. 
   In this case, the surrogate model will have one degree of freedom (the system capacity) and the hours of storage and hot temperature will be parameters set to the the values specified by the user.
...
However, values for hours of storage and hot temperature must still be provided to the model because they are used for costing calculations.
...
From the configuration, the model detects that ``hours_storage`` and ``temperature_hot`` are inputs to the surrogate and
creates them as ``Var`` components rather than ``Param`` components.
...
the inputs, assumptions, and limitations of the PySAM model they are using to generate the data. For example, the example PySAM 
...
though this is not straightforward. While for solar models the steady-state power generation is represented as either the ``electricity`` or ``heat`` variable on
