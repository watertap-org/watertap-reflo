...
This flowsheet simulates the semibatch operation of an LTMED + VAGMD system using the IDAES `multiperiod framework <https://idaes-pse.readthedocs.io/en/latest/reference_guides/apps/grid_integration/multiperiod/index.html>`_, and
   ...
   * each flowsheet incorporates the following models
      ...
      * one :ref:`LT-MED <LT_MED_homepage>` model from WaterTAP-REFLO
      * one :ref:`VAGMD <VAGMD_base_homepage>` model from WaterTAP-REFLO,
...
* Discharging and refilling phase:
   ...
   * The length of discharging and refilling phase is determined by the following equation:
   ...
   where :math:`\nu_{MED,brine,t_{0}}` and :math:`\nu_{MD,dist,t_{0}}` are the volumetric flow rate of MED brine and MD dist 
...
* System scaling
   ...
   The system can be scaled for purposes of cost analysis. The user needs to specify the target system capacity
   ``target_system_capacity``, and the model will calculate the capacity of MED and MD respectively, based on the
...
.. csv-table::
   :header: "Description", "Argument", "Symbol", "Valid Range", "Units"

   "Number of periods in the processing phase", "``n_time_points``", ":math:`N`", "Positive Integer", ":math:`-`"
   "Time interval during processing phase", "``dt``", ":math:`\text{dt}`", "\>0 or ``None``", ":math:`\text{s}`"
   "Feed water temperature into LTMED", "``med_feed_temp``", ":math:`T_{f,med}`", "20 - 30", ":math:`\text{°C}`"
   "Steam temperature into LTMED", "``med_feed_temp``", ":math:`T_{s,med}`", "60 - 80", ":math:`\text{°C}`"
   "Feed water salinity into LTMED", "``med_feed_salinity``", ":math:`S_{f,med}`", "30 - 60", ":math:`\text{g/L}`"
   "MED capacity associated with a single MD module", "``med_capacity``", ":math:`Q_{MED}`", "\>0", ":math:`\text{m}^3\text{/day}`"
   "Feed flow rate of a single VAGMD module", "``md_feed_flow_rate``", ":math:`FFR_{md}`", "400 - 1100", ":math:`\text{L/hr}`"
   "Condenser inlet temperature of VAGMD", "``md_cond_inlet_temp``", ":math:`TCI_{md}`", "20 - 30", ":math:`\text{°C}`"
   "Evaporator inlet temperature of VAGMD", "``md_evap_inlet_temp``", ":math:`TEI_{md}`", "60 - 80", ":math:`\text{°C}`"
   "MD cooling system type", "``md_cooling_system_type``", ":math:`-`", "open or closed", ":math:`-`"
   "MD cooling water inlet temperature", "``md_cooling_inlet_temp``", ":math:`T_{cooling,in}`", "20 - 30", ":math:`\text{°C}`"
...
Notes on configuration inputs:
   ...
   * :math:`Q_{MED}`: MED capacity should be associated with a single MD module for the batch process simulation, before scaled to a certain capacity. This value should be selected so that there's enough MED brine that can be fed to the MD component.
   * :math:`T_{cooling,in}`: The cooling water inlet temperature is not required when cooling system type is set to "closed". See details in the :ref:`VAGMD base model documentation <VAGMD_base_homepage>`.
...
.. csv-table::
   ...
   "Water production from MED during processing phase", ":math:`m_{prod,med,process} = {\sum_{t=1}^{N} m_{dist,t}\text{dt}}`"
   "Water production from MD during processing phase", ":math:`m_{prod,md,process} = {\sum_{t=1}^{N} J_{t} A_{module} \text{dt}}`"
   "Water production from MED during refilling phase", ":math:`m_{prod,med,refill} = m_{dist,t=0} t_{refill}`"
   "Water production from MD during refilling phase", ":math:`m_{prod,md,refill} = J_{t=0} A_{module} t_{refill}`"
   "Total water production during one batch", ":math:`m_{prod,tot,batch} = m_{prod,med,process} + m_{prod,md,process} + m_{prod,med,refill} + m_{prod,md,refill}`"
   "Total operation time of one batch", ":math:`t_{batch,tot} = t_{refill} + N \times \text{dt}`"
   "System capacity associated with one MD module", ":math:`Q_{MED} = \cfrac{m_{prod,tot,batch}}{t_{batch,tot}}`"
   "Total thermal power required", ":math:`P_{th,tot} = \cfrac{({\sum_{t=1}^{N}P_{th,md,t}} + {\sum_{t=1}^{N}P_{th,med,t}}) \times \text{dt} + (P_{th,md,t=0} + P_{th,med,t=0}) \times t_{refill}}{t_{batch,tot}}`"
   "Average STEC of a batch", ":math:`STEC = \cfrac{P_{th,tot} \times t_{batch,tot}}{m_{prod,tot,batch}}`"
...
.. csv-table::
   :header: "Variable In Previous Period", "Symbol", "Variable In Next Period", "Symbol"
   ...
   "Temperature of flow from S2", ":math:`T_{S2}`", "Temperature of liquid remaining in M1", ":math:`T_{M1,remain}`"
   "TDS mass flow rate from S2 and remaining in M1", ":math:`m_{TDS,S2-to-mix}`", "TDS mass flow rate remaining in M1", ":math:`m_{TDS,remain}`"
   "H2O mass flow rate from S2 and remaining in M1", ":math:`m_{H2O,S2-to-mix}`", "H2O mass flow rate remaining in M1", ":math:`m_{H2O,remain}`"
   "Flow pressure from S2", ":math:`P_{S2}`", "Pressure of liquid remaining in M1", ":math:`P_{remain}`"
